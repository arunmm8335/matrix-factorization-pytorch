...
The goal of this project was to reproduce classic algorithms, benchmark their performance on **MovieLens** datasets (Small & 1M), and critique their limitations in a "Research Engineering" context.
...
An implementation of the SVD++ algorithm described in the classic paper **"Matrix Factorization Techniques for Recommender Systems"** by Koren et al. (2009).

* **Key Concept:** Modeling users and items as vectors in a latent factor space. The interaction is modeled as a dot product plus bias terms ($b_u$, $b_i$) to account for systematic tendencies (e.g., some users rate everything high).
* **Optimization:** Stochastic Gradient Descent (SGD) minimizing Regularized Squared Error.
...
### Critique & Analysis
1.  **The Cold Start Problem:** The model failed completely for new users (`IndexError`), confirming the paper's note that collaborative filtering struggles with new entities.
...
### Results (10 Epochs on MovieLens 1M)
| Metric | Result | Notes |
| :--- | :--- | :--- |
| **Final Loss** | **4.3035** | Trained on 1 Million ratings (10x larger dataset). |

### Critique & Analysis
* **Does Order Matter? (The "Time Travel" Test)**
    * **Test:** Fed sequence `[A -> B]` vs `[B -> A]` to see if predictions changed.
    * **Result:**
        * Input Sequence `[100 -> 200]`: Predicted Item **69**
        * Input Sequence `[200 -> 100]`: Predicted Item **148**
    * **Conclusion:** **SUCCESS.** Unlike the static models (which output identical predictions regardless of order), the Transformer successfully learned that the **order of consumption matters**. This confirms that with sufficient data (1M+ ratings), Transformers outperform static matrix factorization by capturing temporal dynamics.
